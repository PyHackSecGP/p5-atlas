...
Every checkpoint shows:
  - What the agent found
  - What it plans to do next
  - WHY (reasoning)
  - What you should look for
Then pauses for approval.
"""
...
class CheckpointResult:
    ...
    def __init__(self, action: str, note: str = "", override: str = ""):
        self.action   = action
        self.note     = note
        self.override = override  # user-provided command override
...
def checkpoint(
    ...
    """Show checkpoint panel and wait for user decision."""
    risk_color = {"low": "green", "medium": "yellow", "high": "red", "critical": "bright_red"}.get(risk, "yellow")
...
def notify(agent: str, message: str, level: str = "info") -> None:
    """Non-blocking status notification."""
    colors = {"info": "cyan", "success": "green", "warning": "yellow", "error": "red"}
    icon   = {"info": "ℹ", "success": "✓", "warning": "⚠", "error": "✗"}
    color  = colors.get(level, "cyan")
    sym    = icon.get(level, "ℹ")
    console.print(f"  [{color}]{sym}[/{color}] [bold dim]{agent}[/bold dim] {message}")
